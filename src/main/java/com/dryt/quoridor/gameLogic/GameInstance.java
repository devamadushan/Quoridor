...
public class GameInstance {
    ...
    public GameInstance(int size, int nbPlayers, int AIPlayers, int walls) {
        ...
        for (int i = 0; i < AIPlayers; i++) {
            ...
            side++;
            players[nbPlayers - i - 1] = new Player("AI" + (AIPlayers + i), pos[1], pos[0], GoalR, GoalD, true);
        }
        ...
        for (int i = 0; i < nbPlayers - AIPlayers; i++) {
            ...
            if (side % 2 == 0) { GoalR = size * 2; }
            side++;
            players[nbPlayers - AIPlayers - i - 1] = new Player("Player" + i, pos[1], pos[0], GoalR, GoalD, false);
        }
    }

    // Ajout d'un getter pour le plateau
